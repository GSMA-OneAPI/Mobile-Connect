...
- (void)getDiscoveryActive: (NSString*)url clientId:(NSString*)clientId clientSecret:(NSString*)clientSecret encryption:(encryptionTypeEnum)encryption ipAddress:(NSString*)ipAddress redirectUri:(NSString*)redirectUri msisdn:(NSString*)msisdn webview: (UIWebView *) webview {
    ...
    if (mcc && mnc) {
        ...
        requestUrl = [requestUrl stringByAppendingString: [NSString stringWithFormat: @"&Identified-MCC=%@&Identified-MNC=%@", [Utils urlEncode:mcc], [Utils urlEncode:mnc]]];
        ...
        if (mcc && mnc) {
            ...
            requestUrl = [requestUrl stringByAppendingString: [NSString stringWithFormat: @"&Selected-MCC=%@&Selected-MNC=%@", [Utils urlEncode:mcc], [Utils urlEncode:mnc]]];
        }else{
    ...
    // - REDIRECT URI -
    if (redirectUri) {
        requestUrl = [requestUrl stringByAppendingString: [NSString stringWithFormat: @"&Redirect_URL=%@", [Utils urlEncode: redirectUri ]]];
    }
    ...
    // - MSISDN -
    if (msisdn) {
        NSString *params = [NSString stringWithFormat: @"MSISDN=%@", [Utils urlEncode: msisdn ]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        ...
        if (error){//Error case in the request process.
            ...
            if (_traceMode) NSLog(@"Error received from server,%@", error);
        ...
        if (error){//Error case in the request process.
            ...
            if( parseError ) {//Error parting JSON
                ...
                if ([urlResponse  statusCode] == 0 ) {//JSON response body, but not status code provided/parsed
                    ...
                    [jsonResponseObj setValue:subscriber_id forKey:@"subscriber_id"];
                    if ([urlResponse  statusCode] == 200 ) {
                        //Save dictionaryResponse to persistent mem
                        [utils saveLastDiscoveryResponse:jsonResponseObj];
                    }
    ...
    @try {
        ...
        for(int i=0;i<[splitResult count];i++){
            ...
            if ([aa count]==2) {
                ...
                if ([[aa objectAtIndex:0] compare:@"subscriber_id" options:NSCaseInsensitiveSearch] == NSOrderedSame ) {
                    if (_traceMode) NSLog(@"Found subscriber_id parameter: %@  --> %@",[aa objectAtIndex:0], [aa objectAtIndex:1]);
                    [jobs setObject: [aa objectAtIndex:1] forKey:[aa objectAtIndex:0]];
                }
            }
...
- (NSString *) encryptedSubscriberID: (NSMutableDictionary *) discoveryData {
    NSString *result=nil;
    if (discoveryData!=nil && discoveryData.count>0) {
        result=[discoveryData objectForKey:@"subscriber_id"];
    }
    return result;
}
    ...
    if ([request.URL.absoluteString hasPrefix:self.redirectUri]) {
        ...
        if (queryItems!=nil && queryItems.count>0) {
            ...
            for (int i=0; i<queryItems.count; i++) {
                ...
                if ([qi.name isEqualToString:@"subscriber_id"]) {
                    subscriber_id=qi.value;
                }
            }
            if (mcc_mnc!=nil) {
                ...
                if ([hniArray count]==2) {
                    ...
                    [self internalPhase2DiscoveryHandler:self.discoveryurl
